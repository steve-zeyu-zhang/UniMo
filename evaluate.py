...
class PointCloudEval:
    def __init__(self, dataloaders, tgt_skeletons, ckpt_path, cfg, device='cpu'):
        ...
        # 创建新的目录结构
        self.bvh_predict_dir = os.path.join(self.epoch_dir, "bvh", "predict")
        self.bvh_gt_dir = os.path.join(self.epoch_dir, "bvh", "GT")
        self.pcd_dir = os.path.join(self.epoch_dir, "pcd")
        os.makedirs(self.bvh_predict_dir, exist_ok=True)
        os.makedirs(self.bvh_gt_dir, exist_ok=True)
    ...
    def evaluate(self, n_tests):
        ...
        for loader_idx, dataloader in enumerate(self.dataloaders):
            with torch.no_grad():
                ...
                for batch_idx in range(batch_size):
                    ...
                    pred_bvh_path = os.path.join(self.bvh_predict_dir, f"bvh_predict_{batch_idx:03d}.bvh")
                    ...
                    gt_bvh_path = os.path.join(self.bvh_gt_dir, f"bvh_GT_{batch_idx:03d}.bvh")
                ...
                pred_pointcloud = tgt_skeleton.generate_pointcloud(pred_global_p_adjusted, pred_global_q,
                                                                   ...
                                                                   std=self.cfg["PC-MRL"]["point_sampling_std"])
                for batch_idx in range(batch_size):
                    batch_dir = os.path.join(self.pcd_dir, f"pcd_{batch_idx:03d}")
        ...
        with open(os.path.join(self.epoch_dir, "evaluation_log.txt"), 'a') as log:
