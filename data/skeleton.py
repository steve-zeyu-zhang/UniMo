...
class Skeleton:
    ...
    def save_bvh(self, filename, root_positions, global_rotations, frame_time=1 / 30.0):
        ...
        Args:
            filename: 输出文件名
            root_positions: [n_frames, 3] 根关节的全局位置
            global_rotations: [n_frames, n_joints, 4] 各关节的全局四元数
            frame_time: 每帧的时间（秒）
        """
        import numpy as np
        from pytorch3d.transforms import matrix_to_euler_angles
