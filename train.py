...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
            ...
            try:
                ...
                if custom_cam != None:
                    ...
                    xyz, features, opacity, scales, rotations, cov3D_precomp, \
                        ...
                        viewpoint_cam.world_view_transform, pipe.compute_cov3D_python, scaling_modifer)
                    net_image = render(custom_cam, xyz, features, opacity, scales, rotations, active_sh_degree,
                                       max_sh_degree, pipe, background, scaling_modifer, cov3D_precomp=cov3D_precomp)
        ...
        xyz, features, opacity, scales, rotations, cov3D_precomp, \
            active_sh_degree, max_sh_degree, masks = scene.get_gaussian_parameters(viewpoint_cam.world_view_transform,
                                                                                   pipe.compute_cov3D_python,
                                                                                   random=random_level)
        render_pkg = render(viewpoint_cam, xyz, features, opacity, scales, rotations, active_sh_degree, max_sh_degree,
                            pipe, background, cov3D_precomp=cov3D_precomp)
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix(
                    {"RGB Loss": f"{ema_loss_for_log:.{4}f}", "Depth Loss": f"{ema_depth_loss_for_log:.{4}f}"})
                progress_bar.update(10)
            if iteration == opt.iterations:
                progress_bar.close()

            # Log and save
            training_report(tb_writer, iteration, rgb_loss, depth_loss, l1_loss, iter_start.elapsed_time(iter_end),
...
def training_report(tb_writer, iteration, rgb_loss, depth_loss, l1_loss, elapsed, testing_iterations, scene: Scene,
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    xyz, features, opacity, scales, rotations, cov3D_precomp, \
                        active_sh_degree, max_sh_degree, masks = scene.get_gaussian_parameters(
                        viewpoint.world_view_transform, renderArgs[0].compute_cov3D_python)
                    results = renderFunc(viewpoint, xyz, features, opacity, scales, rotations, active_sh_degree,
                                         max_sh_degree, cov3D_precomp=cov3D_precomp, *renderArgs)
                    ...
                    if tb_writer and (idx == 0):
                        tb_writer.add_images(config['name'] + "_view_{}/render".format(viewpoint.image_name),
                                             image.unsqueeze(0), global_step=iteration)
                        tb_writer.add_images(config['name'] + "_view_{}/depth".format(viewpoint.image_name),
                                             depth.unsqueeze(0), global_step=iteration)
                        if iteration == testing_iterations[0]:
                            tb_writer.add_images(config['name'] + "_view_{}/ground_truth".format(viewpoint.image_name),
                                                 gt_image.unsqueeze(0), global_step=iteration)
                            if viewpoint.depth is not None:
                                tb_writer.add_images(
                                    config['name'] + "_view_{}/ground_truth_depth".format(viewpoint.image_name),
                                    gt_depth.unsqueeze(0), global_step=iteration)
                for level in range(scene.max_level + 1):
                    viewpoint = config['cameras'][0]  # [randint(0, len(config['cameras'])-1)]
                    xyz, features, opacity, scales, rotations, cov3D_precomp, \
                        active_sh_degree, max_sh_degree, masks = scene.get_gaussian_parameters(
                        viewpoint.world_view_transform, renderArgs[0].compute_cov3D_python, random=level)
                    image = torch.clamp(
                        renderFunc(viewpoint, xyz, features, opacity, scales, rotations, active_sh_degree,
                                   max_sh_degree, cov3D_precomp=cov3D_precomp, *renderArgs)["render"], 0.0, 1.0)
                    if tb_writer:
                        tb_writer.add_images(config['name'] + "_view_{}/level_{}".format(viewpoint.image_name, level),
                                             image.unsqueeze(0), global_step=iteration)

                l1_test = sum(l1_test) / len(l1_test)
                psnr_test = sum(psnr_test) / len(psnr_test)
                print("\n[ Training ] [ITER {}] Evaluating {}: L1 {} PSNR {}".format(iteration, config['name'], l1_test,
                                                                                     psnr_test))
                if tb_writer:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int,
                        default=[50, 100, 500, 1_000, 5_000, 10_000, 20_000, 30_000])
