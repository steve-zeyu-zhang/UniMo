...
def plot_3d_motion(args, figsize=(10, 10), fps=120, radius=4):
    ...
    kinetic_chain = smal_35_kinematic_chain
    limits = 1000 if nb_joints == 21 or nb_joints == 35 else 2
    ...
    def update(index):
        ...
        fig = plt.figure(figsize=(480/96., 320/96.), dpi=96) if nb_joints == 21 or nb_joints == 35 else plt.figure(figsize=(10, 10), dpi=96)
...
def plot_3d_motion_v2(joints, save_path, title, figsize=(10, 10), fps=120, radius=4):
    matplotlib.use('Agg')

    nb_joints = joints.shape[1]
    kinematic_tree = [[0, 11, 12, 13, 14, 15], [0, 16, 17, 18, 19, 20], [0, 1, 2, 3, 4], [3, 5, 6, 7], [3, 8, 9, 10]] if nb_joints == 21 else [[0, 2, 5, 8, 11], [0, 1, 4, 7, 10], [0, 3, 6, 9, 12, 15], [9, 14, 17, 19, 21], [9, 13, 16, 18, 20]]
...
def vis(file_name):
    ...
    motion = np.load(f"dataset/animals_smal_joints/{file_name}.npy")
    outfile = os.path.join('output/vis-tmp', f"{file_name}.gif")
    out = plot_3d_motion([motion, None, file_name])
    if outfile is not None:
        imageio.mimsave(outfile, np.array(out), fps=20)


if __name__ == '__main__':
    file_names = ['foxWDFS_Actions1', 'bear3EP_Jump', 'deerA4K_drink', 'deerK5L_Jump', 'foxZED_death2']
