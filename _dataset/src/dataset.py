...
class dateset_joint_plus_verticesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        vertices = torch.cat([self.joint_list[index].to(self.device), self.vertices_list[index].to(self.device)],
                             dim=1)  # [n_frame,1024+24,3]
        motion_calsses = self.motion_calsses[index]
        cls = self.cls_list[index]
        lengths = self.lengths_list[index].to(self.device)
        # print(lengths)
    ...
    if args.dataset == "uestc":
        ...
        if args.jointstype == "vertices":
            if args.num_animal_classes == 1 and args.pointcloud_in_order == True:
                data_path = os.path.join(PROJECT_ROOT_DIR, "_dataset/data/humanact12_1_vertices_1024points60frames.pkl")
                ...
                data_path = os.path.join(PROJECT_ROOT_DIR,
                                         "_dataset/data/humanact12_unorder1_vertices_1024points60frames.pkl")
            ...
            dataset = dateset_joint_plus_verticesDataset(args=args,
                                                         data_path="./_dataset/data/humanact12_J_joint_vertices_1024points60frames.pkl")
        elif args.jointstype == "mocap_surface":
            dataset = HumanAct12_smpl_VerticesDataset(args=args,
                                                      data_path="./_dataset/data/humanact12_inorder1_vertices_1024points60frames.pkl")
