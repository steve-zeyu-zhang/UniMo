...
def calculate_acc(args, model, evalute_lassifier, iterator, model_save_path, epoch):
    with torch.no_grad():
        ...
        acc, _ = calculate_accuracy(model=None, motion_loader=motion_loader, num_labels=args.num_classes,
                                    classifier=evalute_lassifier, device="cuda:0")
        log_info = "----------------------------------------------------------------"
        log_out_message(model_save_path, log_info)
        log_info = "epoch:%d,acc:%10.8f" % (epoch + 1, acc)
        log_out_message(model_save_path, log_info)
        wandb.log({"val/acc": acc}, step=epoch)
        log_info = "----------------------------------------------------------------"
        log_out_message(model_save_path, log_info)
...
def main():
    ...
    wandb.init(
        project="PCMG-Training",  # 项目名称（自定义）
        name=f"exp-{datetime.datetime.now().strftime('%Y-%m-%d-%H-%M')}",  # 实验名称
        config=vars(args)  # 记录所有超参数
    )
    model = PCMG(args).cuda()
    wandb.watch(model)  # 添加这一行
    ...
    args.save_model_path = "./check_point/" + str(datetime.datetime.now().year) + "_" + str(
        datetime.datetime.now().month) + "_" + str(datetime.datetime.now().day) + "_modelpara.pth"
    ...
    if not continue_train:
        # 需要继续训练的模型的路径
        begin_epoch = 0
        model_save_path = "./check_point/ontraining/" + "_" + str(args.dataset) + "_" + str(
            args.num_frames) + "_" + str(args.rec_loss) + "_" + str(args.jointstype) + "_" + str(
            args.dataset) + "_" + str(datetime.datetime.now()) + "/"
    ...
    if args.evaluate_model == "GRU":
        if (args.jointstype == "smpl"):
            evalute_classifier = load_gru_classifier(args.dataset, args.points_num * 3, args.num_classes,
                                                     device="cuda:0").eval()
        ...
        from evaluate.models.PCMG_classfier import load_classifier as load_P4Transformer_classifier
        evalute_classifier = load_P4Transformer_classifier(args.dataset, args.points_num * 3, args.num_classes,
                                                           device="cuda:0").eval()
    # print("evaluate_model:", args.evaluate_model)
    # print("jointstype:", args.jointstype)
    ...
    for epoch in range(begin_epoch, args.num_epochs):
        ...
        for i, batch in tqdm(enumerate(train_iterator), desc="Computing batch", total=len(train_iterator)):
            ...
            if args.dataset == "uestc":
                batch["xyz"] = rot2xyz(x=batch["xyz"].permute(0, 2, 3, 1).cuda(), mask=batch["mask"], pose_rep='rot6d',
                                       glob=args.glob,
                                       translation=args.translation, jointstype='smpl', vertstrans=args.vertstrans,
                                       betas=None,
            ...
            if args.rec_loss == "cd_density":
                ...
                m_J_regressor_path = "/root/autodl-tmp/UniMo/SMPL_python_v.1.1.0/smpl/models/basicmodel_neutral_lbs_10_207_0_v1.1.0.pkl"
                ...
                vertice_loss = args.lambda_joint * model.compute_vertices_loss(joint_gt, joint_output
                                                                               ) + args.lambda_vertices * model.compute_vertices_loss(
                    vertices_gt, vertices_output
        ...
        wandb.log({
            "train/loss": all_loss,
            "train/vertices_loss": all_loss_vertices,
            "train/kl_loss": all_loss_kl,
            "train/density_loss": all_loss_density,
        ...
        if (smallest_loss > all_loss):
            ...
            wandb.save(ontraining_model_path)
        log_info = "----------------out train-------------------%d个epoch,lr:%10.8f,min_loss:%10.8f" % (
        (epoch + 1), optimizer.state_dict()['param_groups'][0]['lr'], smallest_loss)
        log_out_message(model_save_path, log_info)
        log_info = "loss:%10.8f,vertices loss:%10.8f,density loss:%10.8f,kl loss:%10.8f" % (
        all_loss, all_loss_vertices, all_loss_density, all_loss_kl)
        log_out_message(model_save_path, log_info)
