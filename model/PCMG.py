...
class Encoder(nn.Module):
    ...
    def forward(self, batch):  # [B,L,N,3]->[B,L,latent_dim]
        ...
        # print(y)
        xseq = torch.cat((muQuery[None], sigmaQuery[None], point_encoder_z),
                         axis=0)  # [L,B,latent_dim]->[L+2,B,latent_dim]
...
class Transformer_pointdecoder(nn.Module):
    ...
    def forward(self, batch):
        B, L, N, channel = batch["xyz"].shape
        batch["xyz"] = F.interpolate(batch["xyz"].permute(0, 1, 3, 2),
                                     size=(3,1024),
                                     mode="bilinear",  # 解决 4D 输入的问题
                                     align_corners=False).permute(0, 1, 3, 2)
        latent_z = batch["trans_output"]
        output = torch.empty(size=(latent_z.shape[0], 0, 3)).to(latent_z.device)
        for i in range(self.num_branch):
            _x_1 = latent_z
        ...
        output = output.view(B, L, output.shape[1], channel)
